...
class Model {
  ...
  _initValues(values, options) {
    ...
    if (options.isNewRecord) {
      ...
      if (this.constructor._timestampAttributes.deleted && defaults[this.constructor._timestampAttributes.deleted]) {
        this.dataValues[this.constructor._timestampAttributes.deleted] = Utils.toDefaultValue(defaults[this.constructor._timestampAttributes.deleted], this.sequelize.options.dialect);
        delete defaults[this.constructor._timestampAttributes.deleted];
      }
  ...
  static _paranoidClause(model, options = {}) {
    ...
    const deletedCol = model._timestampAttributes.deleted;
    const deletedAttribute = model.rawAttributes[deletedCol];
    const deletedObject = {};

    let deletedDefaultValue = Object.prototype.hasOwnProperty.call(deletedAttribute, 'defaultValue') ? deletedAttribute.defaultValue : null;

    deletedDefaultValue = deletedDefaultValue || {
      [Op.eq]: false
    };

    deletedObject[deletedAttribute.field || deletedCol] = deletedDefaultValue;

    if (Utils.isWhereEmpty(options.where)) {
      options.where = deletedObject;
    } else {
      options.where = { [Op.and]: [deletedObject, options.where] };
    }
  ...
  static _addDefaultAttributes() {
    ...
    if (this._timestampAttributes.deleted) {
      tail[this._timestampAttributes.deleted] = {
        type: DataTypes.DATE,
        _autoGenerated: true
      };
    }
   ...
   * @param {boolean}                 [options.paranoid=false] Calling `destroy` will not delete the model, but instead set a `deleted` timestamp if this is true. Needs `timestamps=true` to work
   ...
   * @param {string|boolean}          [options.deleted] Override the name of the deleted attribute if a string is provided, or disable it if false. Timestamps must be true. Underscored field will be set with underscored setting.
  ...
  static init(attributes, options = {}) {
    ...
    if (this.options.timestamps) {
      for (const key of ['createdAt', 'updatedAt', 'deleted']) {
      ...
      if (this.options.paranoid && this.options.deleted !== false) {
        this._timestampAttributes.deleted =
          typeof this.options.deleted === 'string' ? this.options.deleted : 'deleted';
        this._readOnlyAttributes.add(this._timestampAttributes.deleted);
      }
    }
   ...
   * @param  {boolean}      [options.force=false]           Delete instead of setting deleted to current timestamp (only applicable if `paranoid` is enabled)
  ...
  static async destroy(options) {
    ...
    if (this._timestampAttributes.deleted && !options.force) {
      // Set query type appropriately when running soft delete
      options.type = QueryTypes.BULKUPDATE;

      const attrValueHash = {};
      const deletedAttribute = this.rawAttributes[this._timestampAttributes.deleted];
      const field = this.rawAttributes[this._timestampAttributes.deleted].field;
      const where = {
        [field]: Object.prototype.hasOwnProperty.call(deletedAttribute, 'defaultValue') ? deletedAttribute.defaultValue : null
      };
  ...
  static async restore(options) {
    if (!this._timestampAttributes.deleted) throw new Error('Model is not paranoid');
    ...
    // Run undelete query
    const attrValueHash = {};
    const deletedCol = this._timestampAttributes.deleted;
    const deletedAttribute = this.rawAttributes[deletedCol];
    const deletedDefaultValue = Object.prototype.hasOwnProperty.call(deletedAttribute, 'defaultValue') ? deletedAttribute.defaultValue : null;

    attrValueHash[deletedAttribute.field || deletedCol] = deletedDefaultValue;
  ...
  */
  ...
  /**
   * Destroy the row corresponding to this instance. Depending on your setting for paranoid, the row will either be completely deleted, or have its deleted timestamp set to the current time.
   *
   * @param {object}      [options={}] destroy options
  ...
  async destroy(options) {
    ...
    let result;
    if (this.constructor._timestampAttributes.deleted && options.force === false) {
      const attributeName = this.constructor._timestampAttributes.deleted;
   ...
   * particularly useful if the implementer renamed the `deleted` attribute
   * to something different.  This method requires `paranoid` to be enabled.
   *
   * @returns {boolean}
   */
  isSoftDeleted() {
    if (!this.constructor._timestampAttributes.deleted) {
      throw new Error('Model is not paranoid');
    }

    const deletedAttribute = this.constructor.rawAttributes[this.constructor._timestampAttributes.deleted];
    const defaultValue = Object.prototype.hasOwnProperty.call(deletedAttribute, 'defaultValue') ? deletedAttribute.defaultValue : null;
    const deleted = this.get(this.constructor._timestampAttributes.deleted) || null;
    const isSet = deleted !== defaultValue;
  ...
  async restore(options) {
    if (!this.constructor._timestampAttributes.deleted) throw new Error('Model is not paranoid');
    ...
    const deletedCol = this.constructor._timestampAttributes.deleted;
    const deletedAttribute = this.constructor.rawAttributes[deletedCol];
    const deletedDefaultValue = Object.prototype.hasOwnProperty.call(deletedAttribute, 'defaultValue') ? deletedAttribute.defaultValue : null;

    this.setDataValue(deletedCol, deletedDefaultValue);
